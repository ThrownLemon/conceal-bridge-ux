...
- **Contrast**: Light mode needs careful checking of "white text on yellow buttons" (might need black text) vs "dark text on yellow buttons".
  - _Mitigation_: The `accent` token in light mode is `amber-600`. We need to ensure text on buttons uses a contrast-safe color.
  - _Refactor_: We likely need to find and replace `bg-slate-950` with a bg token helper.
