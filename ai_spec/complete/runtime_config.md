# Spec: Runtime Configuration (No-Rebuild) — conceal Bridge UX
...
- The app currently injects configuration via [`APP_CONFIG`](conceal-bridge-ux/src/app/core/app-config.ts:17), which is effectively “build-time” because it is compiled into the bundle.
- The build guide notes that environment switching is needed (prod vs testing), especially for `apiBaseUrl` (see current hardcoded value in [`app-config.ts`](conceal-bridge-ux/src/app/core/app-config.ts:20)).
- The environment approach in [`environment_configuration.md`](conceal-bridge-ux/ai_spec/environment_configuration.md:1) solves build-time config, but still requires **rebuild** to change URLs.
...
1. App loads configuration from a static file served alongside the app:
   - [`/config.json`](conceal-bridge-ux/public/config.json:1) (served from [`public/`](conceal-bridge-ux/public:1))
2. Runtime config merges with build-time defaults:
   - Build-time defaults come from environment files (per [`environment_configuration.md`](conceal-bridge-ux/ai_spec/environment_configuration.md:1)).
3. Configuration is available **before** services use it:
   - [`BridgeApiService`](conceal-bridge-ux/src/app/core/bridge-api.service.ts:20) relies on `apiBaseUrl`.
...
4. Provide [`APP_CONFIG`](conceal-bridge-ux/src/app/core/app-config.ts:17) from merged values (environment defaults + runtime overrides)
...
- [`conceal-bridge-ux/public/config.json`](conceal-bridge-ux/public/config.json:1)
...
Add an interface (either colocated with the runtime config service or next to [`AppConfig`](conceal-bridge-ux/src/app/core/app-config.ts:3)):

- `RuntimeConfig` is a partial overlay of [`AppConfig`](conceal-bridge-ux/src/app/core/app-config.ts:3)
...
Best option: keep [`APP_CONFIG`](conceal-bridge-ux/src/app/core/app-config.ts:17) but change its factory to:
...
1. Deploying with a different [`public/config.json`](conceal-bridge-ux/public/config.json:1) changes backend URL **without rebuild**.
2. If config is missing, app still works using build-time defaults.
3. No app code (e.g. [`BridgeApiService`](conceal-bridge-ux/src/app/core/bridge-api.service.ts:13)) needs to change beyond consuming the same [`APP_CONFIG`](conceal-bridge-ux/src/app/core/app-config.ts:17) token.

## Testing Plan

- Local manual:
  1. Create [`public/config.json`](conceal-bridge-ux/public/config.json:1) with a known base URL.
...
- E2E (optional, see [`e2e_testing.md`](conceal-bridge-ux/ai_spec/e2e_testing.md:1)):
  - verify app loads and can route with mocked backend endpoints.

## Risks / Considerations

- Caching: If `config.json` is cached aggressively, changes won’t apply quickly. Ensure correct cache headers (see [`deployment_static_hosting.md`](conceal-bridge-ux/ai_spec/deployment_static_hosting.md:1)).
- CSP: Fetching `/config.json` requires CSP to allow self `connect-src` (see [`security_headers_and_csp.md`](conceal-bridge-ux/ai_spec/security_headers_and_csp.md:1)).
- SSR: Not applicable (this is a pure SPA).

## Implementation Steps (Work Breakdown)

1. Add [`public/config.json`](conceal-bridge-ux/public/config.json:1) template (dev/test friendly).
2. Add [`runtime-config.service.ts`](conceal-bridge-ux/src/app/core/runtime-config.service.ts:1).
3. Add initializer provider(s) in [`app.config.ts`](conceal-bridge-ux/src/app/app.config.ts:7).
4. Update [`APP_CONFIG`](conceal-bridge-ux/src/app/core/app-config.ts:17) factory to merge environment + runtime.
5. Document deployment caching rules for `config.json` in [`deployment_static_hosting.md`](conceal-bridge-ux/ai_spec/deployment_static_hosting.md:1).
