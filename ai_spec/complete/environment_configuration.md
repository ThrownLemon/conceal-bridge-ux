# Spec: Environment Configuration (Prod vs Testing) — conceal Bridge UX
...
- The app currently hardcodes environment-like values inside the injection token factory in [`APP_CONFIG`](conceal-bridge-ux/src/app/core/app-config.ts:17), including:
  - `apiBaseUrl` pointing at a **testing** backend endpoint (see the factory body in [`app-config.ts`](conceal-bridge-ux/src/app/core/app-config.ts:19)).
  - `walletConnectProjectId` checked into source (see [`app-config.ts`](conceal-bridge-ux/src/app/core/app-config.ts:22)).
- Backend requests are built from `apiBaseUrl` inside [`BridgeApiService`](conceal-bridge-ux/src/app/core/bridge-api.service.ts:20).
- There is no `src/environments/` directory in this app today.
- Angular workspace uses Angular 21+ standalone bootstrap via [`bootstrapApplication()`](conceal-bridge-ux/src/main.ts:1) and configurations in [`angular.json`](conceal-bridge-ux/angular.json:1).
...
2. Production build should use **production** backend:
   - `npm run build` → `ng build` should point at production API (default build config is production in [`angular.json`](conceal-bridge-ux/angular.json:56)).
3. No more hardcoded endpoint values in [`APP_CONFIG`](conceal-bridge-ux/src/app/core/app-config.ts:17). It should read from `environment` (file replacement).
4. Keep the current calling shape for the rest of the app:
   - consumers of [`APP_CONFIG`](conceal-bridge-ux/src/app/core/app-config.ts:17) should not need refactors.
...
- Create:
  - [`conceal-bridge-ux/src/environments/environment.ts`](conceal-bridge-ux/src/environments/environment.ts:1) → **production** defaults
  - [`conceal-bridge-ux/src/environments/environment.development.ts`](conceal-bridge-ux/src/environments/environment.development.ts:1) → **testing** defaults (used by `development` build/serve config)
- Update [`angular.json`](conceal-bridge-ux/angular.json:1) `development` configuration to replace `environment.ts` with `environment.development.ts`.
...
Create [`environment.ts`](conceal-bridge-ux/src/environments/environment.ts:1) (production defaults):
...
Create [`environment.development.ts`](conceal-bridge-ux/src/environments/environment.development.ts:1) (testing defaults):
...
Update [`APP_CONFIG`](conceal-bridge-ux/src/app/core/app-config.ts:17) to:

- import `environment` from [`src/environments/environment.ts`](conceal-bridge-ux/src/environments/environment.ts:1)
...
In [`conceal-bridge-ux/angular.json`](conceal-bridge-ux/angular.json:1), for `build.configurations.development`, add:
...
In [`package.json`](conceal-bridge-ux/package.json:1) optionally add explicit scripts for clarity:
...
1. `ng serve` uses the testing backend URL (the same currently hardcoded in [`app-config.ts`](conceal-bridge-ux/src/app/core/app-config.ts:21)).
2. `ng build` uses the production backend URL (previously commented in [`app-config.ts`](conceal-bridge-ux/src/app/core/app-config.ts:20)).
3. No hardcoded env-specific values remain in the [`APP_CONFIG`](conceal-bridge-ux/src/app/core/app-config.ts:17) factory.
4. WalletConnect remains functional and can be configured per environment.
5. All builds compile with strict TS + strict templates (as configured in [`tsconfig.json`](conceal-bridge-ux/tsconfig.json:6) and [`angularCompilerOptions.strictTemplates`](conceal-bridge-ux/tsconfig.json:22)).
...
- Unit test (optional):
  - Add a unit test that injects [`APP_CONFIG`](conceal-bridge-ux/src/app/core/app-config.ts:17) and asserts `apiBaseUrl` equals the expected value for the test environment.
...
- Serve `/config.json` from [`public/`](conceal-bridge-ux/public:1) and fetch it at bootstrap using `APP_INITIALIZER`.
...
1. Create [`src/environments/environment.ts`](conceal-bridge-ux/src/environments/environment.ts:1) and [`src/environments/environment.development.ts`](conceal-bridge-ux/src/environments/environment.development.ts:1).
...
5. Update the docs section “Environment Configuration” in [`angular_build_guide.md`](conceal-bridge-ux/ai_docs/angular_build_guide.md:1) to reflect the new approach.
