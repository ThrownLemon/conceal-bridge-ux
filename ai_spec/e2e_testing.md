# Spec: End-to-End Testing (E2E) — Concael Bridge UX

## Context / Current State

- The project does not have an E2E framework configured (noted in [`README.md`](concael-bridge-ux/README.md:53)).
- Core user journeys are highly flow-based:
  - routing (home → swap) via [`routes`](concael-bridge-ux/src/app/app.routes.ts:3)
  - wallet connect UX via [`WalletButtonComponent`](concael-bridge-ux/src/app/shared/wallet/wallet-button.component.ts:8)
  - swap workflows and polling via [`SwapPage`](concael-bridge-ux/src/app/pages/swap/swap.page.ts:400)
- The app depends on:
  - backend HTTP API ([`BridgeApiService`](concael-bridge-ux/src/app/core/bridge-api.service.ts:13))
  - EIP-1193 wallet provider behaviors (injected or WalletConnect) (see [`EvmWalletService`](concael-bridge-ux/src/app/core/evm-wallet.service.ts:33))
...
Use Playwright network routing to intercept calls generated by [`BridgeApiService`](concael-bridge-ux/src/app/core/bridge-api.service.ts:20):
...
- then return `{ result:true, txdata: {...} }`
  - aligns with [`startPolling()`](concael-bridge-ux/src/app/pages/swap/swap.page.ts:845) filter on `r.result === true`
...
- optionally implement `on` and `removeListener` to support listeners used in [`EvmWalletService.#setProvider()`](concael-bridge-ux/src/app/core/evm-wallet.service.ts:359)

This enables `connect` flows in [`EvmWalletService.connect()`](concael-bridge-ux/src/app/core/evm-wallet.service.ts:134) without user prompts.

### C) Scope: what to test

**Smoke routes**
1. `/` loads home page (contains “Conceal Bridge” from [`HomePage`](concael-bridge-ux/src/app/pages/home/home.page.ts:28))
2. Legacy routes redirect:
   - `/eth` → `swap/ccx-to-evm/eth` per [`app.routes.ts`](concael-bridge-ux/src/app/app.routes.ts:21)
   - `/bsc`, `/plg`, `/ccx` similarly
3. Unknown route shows not-found page ([`NotFoundPage`](concael-bridge-ux/src/app/pages/not-found/not-found.page.ts:4))

**Home → Swap navigation**
4. Select from/to networks and continue (button “Continue” only enabled when wallet is connected per [`HomePage`](concael-bridge-ux/src/app/pages/home/home.page.ts:154))

**Swap page validation**
5. For CCX→EVM:
...
**Polling completion**
7. Start swap, verify UI progresses to completion after mocked polling returns success (step logic in [`SwapPage`](concael-bridge-ux/src/app/pages/swap/swap.page.ts:438))

### D) Test Data Fixtures

Store fixtures under a new folder:
- [`concael-bridge-ux/e2e/fixtures/`](concael-bridge-ux/e2e/fixtures:1)
...
Integrate with the pipeline defined in [`ci_cd_pipeline.md`](concael-bridge-ux/ai_spec/ci_cd_pipeline.md:1):
...
1. Add Playwright dependencies and init config:
   - create [`playwright.config.ts`](concael-bridge-ux/playwright.config.ts:1)
2. Add E2E test folder:
   - [`e2e/`](concael-bridge-ux/e2e:1)
3. Implement backend request interception fixtures.
4. Implement wallet provider injection helper.
5. Add script(s) in [`package.json`](concael-bridge-ux/package.json:4):
   - `e2e`
6. Add CI job step (see [`ci_cd_pipeline.md`](concael-bridge-ux/ai_spec/ci_cd_pipeline.md:1)).