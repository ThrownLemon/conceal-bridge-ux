...
export class ZardDarkMode {
  ...
  private handleSystemChanges(addListener = true): void {
    ...
    try {
      if (addListener) {
        this._query.addEventListener?.('change', this.handleThemeChange);
      } else {
        this._query.removeEventListener?.('change', this.handleThemeChange);
      }
    } catch {
      // Silently ignore - may not be available in test environments
    }
  }
}
